...
with tab2:
    ...
    # 💡 提示：因為時間軸選項變了，原本儲存在資料庫的「第 X 個月」在第一次開啟時可能會讀不到，只要重新點選新月份即可。
    ...
    if st.button("🔄 更新最新即時報價與績效"):
        ...
        if valid_stocks.empty: 
            ...
            with st.spinner('🌐 連線全球交易所與抓取即時匯率中...'):
                ...
                # 更新計算後的總現值到記憶體，交由位於程式碼下方的 Tab 1 直接無縫讀取
